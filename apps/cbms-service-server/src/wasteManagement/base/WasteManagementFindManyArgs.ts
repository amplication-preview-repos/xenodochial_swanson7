/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { WasteManagementWhereInput } from "./WasteManagementWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { WasteManagementOrderByInput } from "./WasteManagementOrderByInput";

@ArgsType()
class WasteManagementFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => WasteManagementWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => WasteManagementWhereInput, { nullable: true })
  @Type(() => WasteManagementWhereInput)
  where?: WasteManagementWhereInput;

  @ApiProperty({
    required: false,
    type: [WasteManagementOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [WasteManagementOrderByInput], { nullable: true })
  @Type(() => WasteManagementOrderByInput)
  orderBy?: Array<WasteManagementOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { WasteManagementFindManyArgs as WasteManagementFindManyArgs };
